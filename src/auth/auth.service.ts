...
export class AuthService {
  ...
  async login(user: UserDto): Promise<ResponseLoginDto> {
    ...
    if (!user.status || user.status === UserDictionary.USER_STATUS_BANNED) {
      throw new HttpException(
        "You don't have permission for this operation. You have been banned/deleted.",
  ...
  async profile(jwtPayloadData: JwtPayloadData): Promise<ProfileDto> {
    ...
    const userProfile: ProfileDto = await this.userService.findOne(
      findByParams.id
    );
